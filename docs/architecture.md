...
- **New stack** → new `sdlc-stack-*` plugin, carrying a `coding-standards-<lang>` skill, a strict
  ...
  `/sdlc:init` and enforced by CI (lint/format/typecheck + a `dependency-cruiser` layering gate).
