...
A complete, copy-paste-ready example of using the Bee-Logical Claude AIDLC on a brand-new
...
## 2. Install the AIDLC plugins (one-time, per developer)

Inside the Claude Code session:

```
/plugin marketplace add D:\AIDLC
/plugin install aidlc@bee-logical
/plugin install aidlc-stack-web@bee-logical
```
...
The **UI/UX design pod** (`aidlc-ux`) is **enabled by default** — no install step needed. It stays
dormant on backend/infra work and wakes up only on UI items, so you'll see it in action at step 6.
Type `/aidlc-ux:` to confirm the `design` command is present.

## 3. Scaffold the AIDLC into the project
...
```
git status          # see the scaffold: CLAUDE.md, .claude/, backlog/, .aidlc/
git add -A && git commit -m "chore: adopt Bee-Logical AIDLC"
```
...
```
/aidlc:intake I want a simple todo app. Users can create a todo with a title,
...
> relations. And `/aidlc:run <free text>` does intake **and** starts building in one step.
...
push branch → open PR → docs**. Progress is written to `.aidlc/runs/TODO-n.md` and commented
...
```
/aidlc:status        # offers cleanup: item → done, run file archived
/aidlc:next          # take the next story
```
...
When `/aidlc:next` reaches the **todos UI** story, the orchestrator detects it's a UI item and — after
...
/aidlc-ux:design app/todos/page.tsx
...
/aidlc-ux:design "redesign the todos page, match design/brand/logo.svg and our brand colors"
```
...
in `.claude/aidlc.config.json` → `ux` (`juryThreshold`, `maxJuryRounds`, `juryPanelSize`, `brand`).
...
- **Next morning**: open Claude Code in the project; it prints where everything stands.
  `/aidlc:run TODO-3` resumes exactly where it stopped.
...
├── .claude/                # AIDLC config, permissions, rules
├── backlog/                # your items: epic + stories, statuses, activity logs
├── .aidlc/runs/             # one auditable run file per item (archived when done)
...
| `gh pr create` fails | `gh auth login`, then `/aidlc:run <ID>` (resumes at the PR step) |
| DB connection refused during tests | Postgres not running / wrong `DATABASE_URL` in `.env` |
| Marketplace add fails with SSH error | The org repo isn't pushed/accessible yet — use the local path `D:\AIDLC` |
