...
rules. Structure is guaranteed three ways: **scaffolded** by `/aidlc:init` (repos start enterprise-
...
exported service). `common/` may not import from `modules/`. See `aidlc-stack-web:nestjs` for the
request-pipeline rules and `aidlc-stack-web:api-design` for the error shape the filter emits.
...
Server Components own server data (fetch in RSC per `aidlc-stack-web:nextjs`). **RTK is for client
...
src/
  ...
  components/
    ui/                       # design-system primitives — consume aidlc-ux tokens; zero business logic
...
## Repo-scaffold checklist (applies to `/aidlc:init` AND any `/aidlc:run` scaffold task)

Whenever a repo is scaffolded to this structure — by `/aidlc:init` Step 4 **or** by an implementer
running a scaffold task via `/aidlc:run` — apply this checklist in full. Items 2–3 are **easy to
forget** because nothing fails loudly without them (there's no CI in local mode to notice a missing
boundary gate), so treat every item as mandatory, not optional. A `/aidlc:run` scaffold has no init
...
4. **Base tsconfig stays strictness-only (F10).** Point the repo's `tsconfig.json` at
   ...
   `aidlc-stack-web:coding-standards-ts` for the why).
5. **Framework extras.** RTK flavors also need `@reduxjs/toolkit` + `react-redux`. **Next.js** repos
   ...
   instead of the plain baseline — see `aidlc-stack-web:nextjs`.
6. **Pre-commit hooks (F21, optional).** The baseline enforces standards at the CI/merge gate but
   ...
   opt-in at `/aidlc:init`; a `/aidlc:run` scaffold should match whatever the sibling repos already do.
