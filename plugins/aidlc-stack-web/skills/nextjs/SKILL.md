...
and store layout live in **`aidlc-stack-web:project-structure`** (`frontend-next-app`), enforced by a
...
**Scaffolding a Next repo?** Follow the repo-scaffold checklist in `aidlc-stack-web:project-structure`
(it applies whether via `/aidlc:init` or a `/aidlc:run` scaffold task): drop the `.dependency-cruiser.cjs`
