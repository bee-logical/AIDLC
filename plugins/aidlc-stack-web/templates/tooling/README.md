...
quality doesn't depend on a reviewer noticing. `/aidlc:init` scaffolds them into a TypeScript repo
...
lint-staged layer closes that gap. It's **opt-in** (some teams decline git hooks) — `/aidlc:init` asks;
