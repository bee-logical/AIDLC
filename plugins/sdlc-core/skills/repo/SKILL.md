...
the folder** in one step, so `/sdlc:run` can route to it and branch into it right away.

Run it in an INTERACTIVE session (it edits `sdlc.config.json` and creates commits). Sub-commands:

## `list`

Read `.claude/sdlc.config.json` → `repos[]` and print each repo: `name`, `path`, `role`, `host`,
...
### 1 · Preconditions
- Read `.claude/sdlc.config.json`. If `workspace.layout` isn't `poly` (or `repos[]` is empty/absent),
...
Same fields as `/sdlc:init` Step 3 (poly): `name`, `path` (relative to `workspace.root`, default
...
3. Lay down an initial payload and **base-commit** it (the pipeline needs at least one commit to branch
   ...
   - **if `sdlc-stack-web` is installed and the repo is TypeScript-based**, the strict tooling baseline
     + enterprise structure skeleton + `.dependency-cruiser.<flavor>.cjs` + hardened `.gitignore`,
     exactly as `/sdlc:init` Steps 4.4–4.6 do (glob `**/sdlc-stack-web/templates/`). Skip for non-TS
     repos.
...
run `/sdlc:run <ID>` for an item that resolves to it." Do NOT commit `sdlc.config.json` automatically —
show `git status` at the control plane and let the user commit the config change.

## Notes
- This is the helper referenced by `sdlc:run` §2.5 (undeclared-repo routing) and `sdlc:init` Step 4
