---
name: scaffold-skill
description: Create a new reusable project-local skill from the SDLC template and register it in the extensions registry. Use when a needed capability (a procedure, convention or integration pattern) does not exist in any installed plugin or local skill and will plausibly be reused.
...
reuse, and only then get promoted to the shared plugin (`/sdlc:promote`). Never write directly
...
2. Project-local `.claude/skills/` and the `.sdlc/extensions.json` registry.
...
2. Instantiate `${CLAUDE_PLUGIN_ROOT}/templates/skill-template/SKILL.md` →
   `.claude/skills/<name>/SKILL.md`. Fill every `{{...}}`; the `x-sdlc` block is mandatory:
...
Append to `.sdlc/extensions.json → extensions[]`:
...
`extensions.json`. `/sdlc:status` surfaces candidates with `reuseCount >= 2` as
promotion-ready. Commit the new skill + registry change with the work item's branch
(`chore(sdlc): scaffold <name> skill`).
