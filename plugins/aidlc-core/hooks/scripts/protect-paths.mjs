...
// Blocks: changes to an EXISTING .claude/settings*.json (creation is allowed — /aidlc:init
...
function block(reason) {
  process.stderr.write(`AIDLC path guard: ${reason}`);
  process.exit(2);
}
...
if (/\/hooks\/(hooks\.json|scripts\/)/.test(p) && /(aidlc-core|\/\.claude\/)/.test(p))
  ...
  process.stdout.write(
    JSON.stringify({
      hookSpecificOutput: {
        ...
        permissionDecisionReason: `AIDLC: modifying CI pipeline file ${p} — confirm this is intended for the current work item.`,
      },
    })
  );
  process.exit(0);
