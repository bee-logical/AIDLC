...
gate the pipeline runs during `/sdlc:run` is *local* — unless a CI job runs the same checks as a
...
`/sdlc:init` offers to scaffold the matching one per **remote** repo; `/sdlc:init` and `/sdlc:status`
...
## Azure Pipelines gotchas (fresh orgs bite here — see `sdlc:ci-cd`)
...
CI uses** (e.g. a `node:22` container) — don't loosen the CI to `npm install`. See `sdlc:ci-cd`.
...
nothing). Pin **`dependency-cruiser@^17`** (the devDep floor lives in `sdlc-stack-web:project-structure`)
...
before fanning a shared-config pattern out to consumers (see `sdlc:ci-cd` and `sdlc:run` poly pilot).
